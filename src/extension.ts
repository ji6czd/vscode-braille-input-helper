...
class BrailleController {
  ...
  private currentChord: number = 0; // 現在押されている点のビット和（bit 0x100 = スペース）
  ...
  constructor(private context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand("braille-input-helper.toggle", () =>
        this.toggle(),
      ),
      vscode.commands.registerCommand(
        "braille-input-helper.input",
        (dot: number) => this.handleInput(dot),
      ),
  ...
  /**
   * 通常の文字入力を制御する
   * 点字モードON時は、改行以外の文字入力をブロックする
   * （スペースは keybinding 経由で handleInput() が処理する）
   */
  private handleType(args: { text: string }) {
    // モードがOFF、または入力がホワイトスペース（改行、タブ等）の場合は通常通り入力させる
  ...
  private playSound(filename: string) {
    ...
    if (process.platform === "win32") {
      // Windows: VBScriptを使用
      const vbsPath = this.context.asAbsolutePath(
        path.join("audio", "play.vbs"),
      );
      executable = "cscript";
   ...
   *            スペースキーは bit 0x100 を渡す（点字パターン U+2800〜U+28FF の外側）。
   *            将来の Space+F のような同時押しコードもこのビット和で表現する。
   */
  ...
  private commitCharacter() {
    ...
    // 下位8ビットが点字パターン、bit 0x100 はスペースフラグ（将来の Space+F 判定用）
    // 点字パターンのUnicodeコードポイントを計算 (U+2800 が空の点字 = スペース)
    const charCode = 0x2800 + (this.currentChord & 0xff);
